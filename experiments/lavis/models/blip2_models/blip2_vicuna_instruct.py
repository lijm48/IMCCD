...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            inputs_embeds = torch.cat([inputs_llm, inputs_embeds], dim=1)
            attention_mask = torch.cat([atts_llm, llm_tokens.attention_mask], dim=1)
            attention_mask_cd = None
            ## cd_comments: format
            if use_icd:
                # import pdb;pdb.set_trace()
                inputs_embeds_cd = self.llm_model.get_input_embeddings()(llm_tokens_cd.input_ids)
                inputs_embeds_cd = torch.cat([inputs_llm.clone(), inputs_embeds_cd], dim=1)
                attention_mask_cd = torch.cat([atts_llm_cd, llm_tokens_cd.attention_mask], dim=1)
            elif images_cd is not None:
            ...
            key_pos = [{"image_token_start": 0,  "image_token_end": inputs_llm.shape[1]}]
