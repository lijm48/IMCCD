...
from transformers.modeling_outputs import BaseModelOutputWithPastIMCCD, CausalLMOutputWithPastIMCCD
...
class LlamaAttention(nn.Module):
    ...
    def modi_weights(self, attn_weights, key_pos):
        ...
        attn_weights[:, :, image_token_end:, image_token_start:image_token_end] = attn_weights[:, :, image_token_end:, image_token_start:image_token_end] - mask * 3.4028e+38
        # attn_weights[:, :, image_token_end:, image_token_start:image_token_end] = mean * mask + attn_weights[:, :, image_token_end:, image_token_start:image_token_end] * (1 - mask)
        return attn_weights
    ...
    def values_noise_multiply(self, attn_weights, key_pos, value_states,  attn_weights_all = None, zero_noise = True):
        try:
            image_token_start = key_pos[0]['image_token_start']
            image_token_end = key_pos[0]['image_token_end']
        except:
        ...
        if zero_noise:
            ...
            # mean = value_states.mean(dim=(3), keepdim = True)
            # noise_value_states = mean.repeat(1, 1, 1, value_states.shape[3])
        else:
        ...
        return attn_output_ori + attn_output_noise, final_mask
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(LLAMA_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=CausalLMOutputWithPastIMCCD, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        key_pos:  Optional[list] = [],
        use_key_pos: Optional[bool] = False,
        modi_pos: Optional[bool] = False,
        ...
        return CausalLMOutputWithPastIMCCD(
            ...
            past_key_values_without_pos= outputs.past_key_values,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        )

    def prepare_inputs_for_generation(
        self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, key_pos = None, **kwargs
    ):
        if past_key_values:
    ...
    def prepare_inputs_for_generation_cd(
        self, input_ids, past_key_values=None, attention_mask=None, attention_mask_cd = None, inputs_embeds=None, use_mask = False,  key_pos = [],  **kwargs
    ):
        if use_mask:
            # import pdb;pdb.set_trace()
        ...
        # if `inputs_embeds` are passed, we only want to use them in the 1st generation step
        if inputs_embeds is not None and past_key_values is None:
            model_inputs = {"inputs_embeds": kwargs.get("images_cd")}
            if use_mask and input_ids.shape[1] > 1:
                generated_embeds = self.get_input_embeddings()(input_ids[:, 1:])
                model_inputs["inputs_embeds"] = torch.cat((model_inputs["inputs_embeds"], generated_embeds), dim=1)
        else:
            model_inputs = {"input_ids": input_ids}
