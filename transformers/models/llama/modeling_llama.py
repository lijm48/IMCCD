...
from ...modeling_outputs import BaseModelOutputWithPastIMCCD, CausalLMOutputWithPastIMCCD, SequenceClassifierOutputWithPast
...
def apply_rotary_pos_emb_part(q, k, cos, sin, position_ids, key_pos):
    try:
    ...
    assert d == 64
    # import pdb;pdb.set_trace()
    k_embed[... , image_token_start: image_token_end, 48:64] = k_embed[... , image_token_start: image_token_end, 48:64] * 1.1
    k_embed[... , image_token_start: image_token_end, 112:128] = k_embed[... , image_token_start: image_token_end, 112:128] * 1.1
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.attention_adapter = AttentionAdapter()
    ...
    def GMM_mask(self, saliency, trim = False, head_wise = False, token_wise = False, high_thres = False):
        data = saliency
        if trim:
            ...
            # median = torch.mean(saliency)

            # 计算中位绝对偏差（MAD）
            # mad = np.median(np.abs(data - median))
            # mad = torch.std(sailency)
    ...
    def modi_weights(self, attn_weights, key_pos):
        try:
            image_token_start = key_pos[0]['image_token_start']
            image_token_end = key_pos[0]['image_token_end']
        except:
            import pdb;pdb.set_trace()
        img_attn = attn_weights[:, :, image_token_end:, image_token_start:image_token_end]
        mask, mean, std = self.GMM_mask(img_attn)
        attn_weights[:, :, image_token_end:, image_token_start:image_token_end] = attn_weights[:, :, image_token_end:, image_token_start:image_token_end] - mask * 65504
        return attn_weights



    def modi_values_noise(self, value_states, key_pos):
        try:
            image_token_start = key_pos[0]['image_token_start']
            image_token_end = key_pos[0]['image_token_end']
        except:
            import pdb;pdb.set_trace()
        img_value_states = value_states[:, :, image_token_start:image_token_end]
        img_value_states  = self.add_diffusion_noise(img_value_states, 999, 0.01)
        value_states[:, :, image_token_start:image_token_end] = img_value_states 
        return value_states
    ...
    def add_diffusion_noise(self, image_tensor, noise_step, beta=0.5e-2, dim_wise = False):
        ...
        if dim_wise:
            mean = torch.mean(image_tensor, dim=(0, 1, 2), keepdim=True)
            std = torch.std(image_tensor, dim=(0, 1, 2), keepdim=True)
        else:
    ...
    def modi_hidden_states_noise(self,  hidden_states, key_pos):
        try:
            image_token_start = key_pos[0]['image_token_start']
            image_token_end = key_pos[0]['image_token_end']
        except:
            import pdb;pdb.set_trace()
        tmp_hidden_states = hidden_states[:, :,  image_token_start:image_token_end]
        tmp_hidden_states = self.add_diffusion_noise(tmp_hidden_states.clone(), 999, 0.01)  
        assert tmp_hidden_states.sum() != torch.inf
    ...
    def estimate_attention_without_pos_part(self, query_states, key_states, attention_mask):
        query_states = torch.cat((query_states[..., 32:64], query_states[..., 96:128]), dim=3)
        key_states = torch.cat((key_states[..., 32:64],  key_states[..., 96:128]), dim=3)
    ...
    def modi_pos(self, position_ids, key_pos, use_past = False):
        try:
            image_token_start = key_pos[0]['image_token_start']
            image_token_end = key_pos[0]['image_token_end']
        except:
    ...
    def forward(
        ...
        if modi_pos:
            ...
            # attn_weights_all = self.estimate_attention_without_pos(query_states.clone(), key_states.clone(), attention_mask)
            attn_weights_all = None
        ...
        if modi_pos:
            past_key_value_without_pos = (tmp_key_states, value_states) if use_cache else None
        ...
        if not output_attentions:
            attn_weights = None
        if modi_pos:
            return attn_output, attn_weights_all, past_key_value, past_key_value_without_pos
        elif adaptive_mask:
            return attn_output, attn_weights_all, past_key_value
        return attn_output, attn_weights_all, past_key_value
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if use_cache:
            ...
            if modi_pos:
                outputs += (present_key_value_without_pos,)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        next_decoder_cache_without_pos = () if use_cache else None
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if past_key_values is not None and idx < len(past_key_values_without_pos):
                past_key_value_without_pos = past_key_values_without_pos[idx] 
            else:
                past_key_value_without_pos = None                
            ...
            if modi_pos:
                # import pdb;pdb.set_trace()
                next_decoder_cache_without_pos += (layer_outputs[cnt + 1],)
        ...
        next_cache_without_pos = next_decoder_cache_without_pos if use_cache else None
        if not return_dict:
            return tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns] if v is not None)
        return BaseModelOutputWithPastIMCCD(
            last_hidden_state=hidden_states,
            past_key_values=next_cache,
            past_key_values_without_pos = next_decoder_cache_without_pos,
